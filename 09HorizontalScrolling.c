...
int main()
{
    ...
    int visibleColumns = maximumColumn;
    ...
    while(true) {
        ...
        for(int screenRow = 0; screenRow < visibleRows; screenRow++) {
            ...
            mvprintw(screenRow, leftMargin, "%.*s", visibleColumns - leftMargin - 1, &textBuffer[bufferRow][leftVisibleColumn]);
        }
        ...
        move(row - topVisibleRow, column - leftVisibleColumn + leftMargin);
        refresh();
